...
func load_scene(scene_path: String) -> void:
	...
	var state: int = ResourceLoader.load_threaded_request(scene_path, "", _use_sub_threads)
	if state == OK:
		_loading_path = scene_path
		current_state = State.LOADING


func _check_loading_state() -> void:
	...
	match load_status:
		...
		ResourceLoader.THREAD_LOAD_LOADED:
			var new_scene: Node = ResourceLoader \
					.load_threaded_get(_loading_path) \
					.instantiate()
			_switch_scenes(new_scene)
			_loading_path = ""


func _switch_scenes(new_scene: Node) -> void:
